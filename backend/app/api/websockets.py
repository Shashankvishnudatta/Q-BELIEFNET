...
from app.core.contracts import infer_market_provenance, make_provenance
from app.services.evidence_pipeline import provider_status_payload
from app.services.background_refresh import ingestion_status_payload
...
async def publish_message(data: dict):
    ...
    data.setdefault(
        ...
        make_provenance(
            source="generated",
            provider="websocket-demo-stream",
            notes="Generated WebSocket demo stream message.",
        ).model_dump(),
    )
    ...
    while True:
        ...
        await publish_message({
            ...
            "meta": make_provenance(
                source="generated",
                provider="websocket-latency-probe",
                notes="Generated latency probe for connection health.",
            ).model_dump(),
        })
    ...
    while True:
        ...
        for client in list(connected_clients):
            client.enqueue({
                ...
                "meta": make_provenance(
                    source="generated",
                    provider="websocket-heartbeat",
                    notes="Generated heartbeat for connection health.",
                ).model_dump(),
            })
...
async def background_task():
    ...
    while True:
        ...
        trending_meta = infer_market_provenance(
            updated_trending,
            fallback_provider="websocket-demo-stream",
            live_provider="rapidapi-yahoo-finance",
        ).model_dump()
        await publish_message({
            "type": "trending_update",
            "payload": updated_trending,
        ...
        stock_meta = infer_market_provenance(
            stock_data,
            fallback_provider="websocket-demo-stream",
        ...
        belief_snapshot = await build_belief_snapshot_async("NVDA")
        belief_meta = belief_snapshot.meta.model_dump()
        await publish_message({
            "type": "belief_snapshot",
            "symbol": belief_snapshot.asset.symbol,
            "payload": belief_snapshot.model_dump(),
            "meta": belief_meta,
        })
        await publish_message({
            "type": "belief_delta",
            "symbol": belief_snapshot.asset.symbol,
            "payload": {
                **belief_snapshot.delta.model_dump(),
                "source_mix": belief_snapshot.source_mix,
                "freshness_summary": belief_snapshot.freshness_summary.model_dump() if belief_snapshot.freshness_summary else None,
                "provider_status_summary": [
                    ...
                    for result in belief_snapshot.provider_results
                ],
                "fallback_used": bool(belief_snapshot.source_mix.get("fallback")),
            },
            "meta": belief_meta,
        })
        if belief_snapshot.narratives:
            await publish_message({
                "type": "narrative_shift",
                "symbol": belief_snapshot.asset.symbol,
                "payload": belief_snapshot.narratives[0].model_dump(),
                "meta": belief_meta,
            })
        await publish_message({
            "type": "system_status",
            "payload": {
                ...
                "belief_stream": "prototype",
                "provider_health": provider_status_payload().get("health_summary", {}),
                "ingestion": {
                    "scheduler_enabled": ingestion_status_payload().get("scheduler_enabled"),
                    "last_run": ingestion_status_payload().get("last_run"),
                },
            },
            "meta": make_provenance(
                source="generated",
                provider="websocket-system-status",
                notes="Generated WebSocket system status for runtime transparency.",
            ).model_dump(),
        })
        ...
        await publish_message({
            ...
            "meta": make_provenance(
                source="generated",
                provider="websocket-provider-health",
                notes="Generated provider health event for pipeline observability.",
            ).model_dump(),
        })
        await publish_message({
            "type": "cache_refreshed",
            "payload": {
                "cache": provider_payload.get("evidence_cache", {}),
                "source_mix": belief_snapshot.source_mix,
                "freshness_summary": belief_snapshot.freshness_summary.model_dump() if belief_snapshot.freshness_summary else None,
            },
            "meta": make_provenance(
                source="generated",
                provider="websocket-cache-observability",
                notes="Generated cache observability event for pipeline transparency.",
            ).model_dump(),
        })
        ...
        while True:
            ...
            try:
                ...
                if msg_type == "ack":
                    ...
                    if channel not in {"trending", "stock", "belief"}:
                        client.enqueue({
                            "type": "error",
                            "payload": {
                                "code": "WEBSOCKET_SUBSCRIPTION_ERROR",
                                "message": "Unsupported subscription channel.",
                            },
                            "meta": make_provenance(
                                source="generated",
                                provider="websocket-server",
                                notes="Subscription error generated by WebSocket server.",
                            ).model_dump(),
                        })
                ...
                client.enqueue({
                    ...
                    "meta": make_provenance(
                        source="generated",
                        provider="websocket-server",
                        notes="Parse error generated by WebSocket server.",
                    ).model_dump(),
                })
