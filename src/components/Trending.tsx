...
export function Trending() {
  const { trendingStocks, isLoadingTrending, trendingMeta, trendingError, fetchTrending, setActiveTicker } = useAppStore();
  ...
  return (
    <div className="flex flex-col h-full w-full px-4 sm:px-8 py-8 max-w-7xl mx-auto overflow-y-auto">
      ...
      >
        <div>
          ...
          <div className="flex flex-wrap items-center gap-2">
            <p className="text-muted-foreground">Top assets by prototype belief velocity.</p>
            <DataSourceBadge meta={trendingMeta} />
          </div>
        </div>
      ...
      {isLoadingTrending ? (
        ...
        <>
        <FallbackNotice meta={trendingMeta} error={trendingError} />
        <motion.div 
          initial={{ opacity: 0 }}
          animate={{ opacity: 1 }}
