...
export function Signals() {
  const { signals, isLoadingSignals, signalsMeta, signalsError, fetchSignals } = useAppStore();
    ...
    <div className="flex flex-col h-full w-full px-4 sm:px-8 py-8 max-w-6xl mx-auto overflow-y-auto">
      <motion.div initial={{ opacity: 0, y: 16 }} animate={{ opacity: 1, y: 0 }} className="mb-8">
        ...
        <div className="flex flex-wrap items-center gap-2">
          <p className="text-muted-foreground">Prototype narrative shifts and sentiment divergences.</p>
          <DataSourceBadge meta={signalsMeta} />
        </div>
      </motion.div>
      ...
      {isLoadingSignals ? (
        ...
        <div className="space-y-6 pb-20">
          <FallbackNotice meta={signalsMeta} error={signalsError} />
          <SignalSection
