...
export function Alerts() {
  const { alerts, isLoadingAlerts, alertsMeta, alertsError, fetchAlerts } = useAppStore();
    ...
    <div className="flex flex-col h-full w-full px-4 sm:px-8 py-8 max-w-6xl mx-auto overflow-y-auto">
      <motion.div initial={{ opacity: 0, y: 16 }} animate={{ opacity: 1, y: 0 }} className="mb-8 flex flex-wrap items-center justify-between gap-4">
        <div>
          ...
          <div className="flex flex-wrap items-center gap-2">
            <p className="text-muted-foreground">Prototype threshold and anomaly alerts across tracked names.</p>
            <DataSourceBadge meta={alertsMeta} />
          </div>
        </div>
      ...
      {isLoadingAlerts ? (
        ...
        <div className="space-y-3 pb-20">
          <FallbackNotice meta={alertsMeta} error={alertsError} />
