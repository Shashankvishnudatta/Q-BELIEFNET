...
import { ApiStatusCard, RuntimeModeBadge } from '@/src/components/DataSourceBadges';
...
const MENU_ITEMS: { icon: LucideIcon; label: string; view: ViewState }[] = [
...
function findTicker(query: string): string | null {
  const matches = query.toUpperCase().match(/\b[A-Z]{1,5}\b/g) ?? [];
  return matches.find((token) => KNOWN_TICKERS.has(token)) ?? null;
}

function buildChatReply(query: string, onNavigate: (view: ViewState) => void, onOpenTicker: (ticker: string) => void): string | null {
  const trimmed = query.trim();
  const normalized = trimmed.toLowerCase();

  const ticker = findTicker(trimmed);
  if (ticker) {
    onOpenTicker(ticker);
    return `Opening ${ticker}. I am pulling the latest available belief score, signal, and chart context now.`;
  }
...
function HomeView({ onNavigate, onOpenTicker }: HomeViewProps) {
  const { trendingStocks, fetchTrending, apiStatus, apiStatusError } = useAppStore();
  ...
  const handleSend = async () => {
    ...
    try {
      const response = buildChatReply(trimmed, onNavigate, onOpenTicker) ?? await queryMarketLLM(trimmed);
      setMessages((current) => {
      ...
      {!hasMessages ? (
        ...
        <div className="flex-1 flex flex-col items-center justify-center px-4 sm:px-6 md:px-8 max-w-5xl mx-auto w-full pb-12">
          <div className="w-full max-w-2xl mb-6">
            <ApiStatusCard status={apiStatus} error={apiStatusError} />
          </div>
          <motion.div 
          ...
          >
            <h1 className="text-3xl sm:text-4xl md:text-5xl font-semibold tracking-tight mb-3 sm:mb-4 text-foreground/90">
              Inspect what the market appears to believe <br className="hidden sm:block" />
              <span className="text-gradient">with evidence and provenance.</span>
            </h1>
            <p className="text-sm text-muted-foreground max-w-2xl mx-auto">
              Q-Belief Net is a belief-signal analytics prototype. It does not provide financial advice or buy/sell recommendations.
            </p>
            {apiStatus && !apiStatus.llm.configured ? (
              <p className="text-sm text-amber-200">
                Hosted AI is not configured. Local belief-aware fallback analysis remains available.
              </p>
            ) : null}
          </motion.div>
          ...
          <div className="border-t border-white/5 bg-background/50 backdrop-blur-sm">
            ...
            <div className="px-4 sm:px-6 md:px-8 py-2 flex items-center justify-between gap-2 text-xs flex-wrap">
              ...
              <Badge variant="secondary" className="bg-teal-500/10 text-teal-400 border-teal-500/20 text-xs">
                {llmFallbackActive ? 'Local fallback' : trendingStocks.length > 0 ? 'Hybrid data ready' : 'Loading context'}
              </Badge>
            </div>
          </div>
        </div>
...
export default function App() {
  const { activeView, setActiveView, wsConnected, setActiveTicker, connectWebSocket, hydrateWatchlist, fetchApiStatus, apiStatus } = useAppStore();
  ...
  }, [connectWebSocket, fetchApiStatus, hydrateWatchlist]);
        ...
        <header className="h-16 flex items-center justify-between px-4 sm:px-6 gap-3 shrink-0 border-b border-border/30 md:border-b-0">
          ...
          <div className="flex items-center gap-2 sm:gap-4 min-w-0">
            <div className="flex items-center gap-2">
              ...
              <span className="text-xs text-muted-foreground hidden sm:inline">{wsConnected ? 'Stream' : 'Static'}</span>
            </div>
            <RuntimeModeBadge status={apiStatus} />
